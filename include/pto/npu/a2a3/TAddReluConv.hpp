...
PTO_INTERNAL void AddReluConvCall(__ubuf__ typename TileDataDst::DType *dst, __ubuf__ typename TileDataSrc::DType *src0,
                                  ...
                                  uint8_t src0RepeatStride, uint8_t src1RepeatStride)
{
    ...
    if constexpr (std::is_same<ST, float>::value && std::is_same<DT, half>::value) {
        vaddreluconv_f322f16(dst, src0, src1, repeat, dstBlockStride, src0BlockStride, src1BlockStride, dstRepeatStride,
                             src0RepeatStride, src1RepeatStride, false);
    } else if constexpr (std::is_same<ST, half>::value && std::is_same<DT, int8_t>::value) {
        vaddreluconv_f162s8(dst, src0, src1, repeat, dstBlockStride, src0BlockStride, src1BlockStride, dstRepeatStride,
                            src0RepeatStride, src1RepeatStride, false);
    } else if constexpr (std::is_same<ST, int16_t>::value && std::is_same<DT, int8_t>::value) {
        vaddreluconv_s162s8(dst, src0, src1, repeat, dstBlockStride, src0BlockStride, src1BlockStride, dstRepeatStride,
                            src0RepeatStride, src1RepeatStride, false);
    }
}
...
// ============================================================================
template <typename TileDataDst, typename TileDataSrc>
PTO_INTERNAL void ComputeAddReluConvConfig(unsigned &elementsPerRepeat, unsigned &dstRepeatStride,
...
__tf__ PTO_INTERNAL void TAddReluConv(typename TileDataDst::TileDType __out__ dstData,
                                      typename TileDataSrc::TileDType __in__ src0Data,
                                      typename TileDataSrc::TileDType __in__ src1Data, unsigned numRepeatPerLine,
                                      unsigned numRemainPerLine, unsigned validRow, unsigned elementsPerRepeat,
    ...
    if (numRepeatPerLine > 0) {
        ...
        for (unsigned i = 0; i < validRow; i++) {
            ...
            if (remainAfterLoop > 0) {
                ...
                AddReluConvCall<TileDataDst, TileDataSrc>(dstPtr + i * DS + span, src0Ptr + i * SS0 + span,
