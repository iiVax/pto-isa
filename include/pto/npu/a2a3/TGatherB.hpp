...
PTO_INTERNAL void GatherBlockHead(__ubuf__ T *dstPtr, __ubuf__ uint32_t *offsetPtr, uint32_t &srcAddr,
                                  unsigned validRow, unsigned numRepeatPerLine)
{
        ...
        if (remainAfterLoop) {
            if constexpr (sizeof(T) == 1) {
                ...
                Op::GatherBInstr((__ubuf__ T *)(dstPtr + i * dstRowStride + numLoop * elementsPerRepeat * REPEAT_MAX),
                                 offsetPtr + i * offsetRowStride + numLoop * 8 * REPEAT_MAX, srcAddr, 8,
                                 remainAfterLoop);
            }
        }
    }
...
template <typename Op, typename T, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned dstRowStride,
          unsigned offsetRowStride>
PTO_INTERNAL void GatherBlockTail(__ubuf__ T *dstPtr, __ubuf__ uint32_t *offsetPtr, uint32_t &srcAddr,
        ...
        for (int i = 0; i < numLoop; i++) {
            if constexpr (sizeof(T) == 1) {
                Op::GatherBInstrB8((__ubuf__ uint16_t *)(dstPtr + i * REPEAT_MAX * dstRowStride),
                                   offsetPtr + i * REPEAT_MAX * offsetRowStride, srcAddr, dstRowStride / blockSizeElem,
                                   REPEAT_MAX);
            } else {
    ...
    if (remainAfterLoop) {
        if constexpr (sizeof(T) == 1) {
            Op::GatherBInstrB8((__ubuf__ uint16_t *)(dstPtr + numLoop * REPEAT_MAX * dstRowStride),
                               offsetPtr + numLoop * REPEAT_MAX * offsetRowStride, srcAddr,
                               dstRowStride / blockSizeElem, remainAfterLoop);
        } else {
...
template <typename Op, typename T, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned dstRowStride,
          unsigned offsetRowStride>
PTO_INTERNAL void TGatherBlock(__ubuf__ T *dstPtr, __ubuf__ uint32_t *offsetPtr, uint32_t &srcAddr, unsigned validRow,
                               unsigned validCol)
{
